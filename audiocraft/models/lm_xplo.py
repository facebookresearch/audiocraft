...
#                            triple_cfg: bool = False,
#                            cfg_coef: tp.Optional[float] = None,
#                            cfg_coef_2: tp.Optional[float] = None,
#                            cfg_coef_3: tp.Optional[float] = None,
#                            ) -> torch.Tensor:
...
#         """
#         # print("CFG_CONDITIONS_SAMPLE_NEXT_TOK", cfg_conditions)
#         # print('CFG_COEFS', cfg_coef, cfg_coef_2,cfg_coef_3, triple_cfg)
#         B = sequence.shape[0]
#         cfg_coef = self.cfg_coef if cfg_coef is None else cfg_coef
#         model = self if self._fsdp is None else self._fsdp
#         if triple_cfg:
#             assert isinstance(cfg_conditions, dict)
#             assert (cfg_coef_2 is not None) and (cfg_coef_3 is not None)
#             condition_tensors = cfg_conditions
#             if condition_tensors:
#                 sequence = torch.cat([sequence, sequence, sequence, sequence], dim=0)
#             all_logits = model(
#                 sequence,
#                 conditions=[], condition_tensors=condition_tensors)
#             if condition_tensors:
#                 cond_logits, wav_logits, text_logits, uncond_logits = all_logits.split(B, dim=0)  # [B, K, T, card]
#                 logits = 1/3 * (uncond_logits + cfg_coef * (cond_logits - uncond_logits) + \
#                                 wav_logits + cfg_coef_2 * (cond_logits - wav_logits) + \
#                                 text_logits + cfg_coef_3 * (cond_logits - text_logits))
#         elif double_cfg:
#             assert isinstance(cfg_conditions, dict)
...
#         elif self.two_step_cfg and cfg_conditions != {}:
...
#                  description_tokens: torch.Tensor = None,
#                  return_conditions: bool = False,
#                  description_tokens_is_cfg: bool = False,
#                  wav_tokens: torch.Tensor = None,
#                  ) -> torch.Tensor:
...
#             double_cfg (bool): if True, we apply a double classifier free guidance and the logits formula is
#                 logits_unc + cfg_coef*[logits_wav + cfg_coef_2*(logits_wav_description - logits_wav) - logits_unc]
...
#         assert (conditions == []) or (description_tokens is None)
...
#         if triple_cfg:
#             num_conditions = 4
#             assert not double_cfg
#             if conditions:
#                 wav_conditions = AttributeDropout(p={'text':{'description': 1.0}, 
#                                                      'wav':{'self_wav': 0.0}})(conditions)
#                 text_conditions = AttributeDropout(p={'text':{'description': 0.0}, 
#                                                      'wav':{'self_wav': 1.0}})(conditions)
#                 null_conditions = ClassifierFreeGuidanceDropout(p=1.0)(conditions)
#                 conditions = conditions + wav_conditions + text_conditions + null_conditions
#                 tokenized = self.condition_provider.tokenize(conditions)
#                 cfg_conditions = self.condition_provider(tokenized)
#             elif description_tokens is not None:
#                 if description_tokens_is_cfg:
#                     cond_token = description_tokens
#                 else:
#                     zeros = torch.zeros_like(description_tokens)
#                     cond_token = torch.cat([description_tokens, zeros, description_tokens, zeros])
#                 cond_mask = torch.zeros(cond_token.shape[:2])
#                 cfg_conditions['description'] = (cond_token, cond_mask)
#             if wav_tokens is not None:
#                 zeros_wav = torch.zeros_like(wav_tokens)
#                 cond_wav_token = torch.cat([wav_tokens, wav_tokens, zeros_wav, zeros_wav])
#                 cond_wav_mask = torch.zeros(cond_wav_token.shape[:2])
#                 cfg_conditions['self_wav'] = (cond_wav_token, cond_wav_mask)

#         elif double_cfg:
#             num_conditions = 3
#             assert not triple_cfg
#             if conditions:
#                 wav_conditions = AttributeDropout(p={'text':{'description': 1.0}, 
#                                                      'wav':{'self_wav': 0.0}})(conditions)
#                 null_conditions = ClassifierFreeGuidanceDropout(p=1.0)(conditions)
...
#                 else:
#                     zeros = torch.zeros_like(description_tokens)
#                     cond_token = torch.cat([description_tokens, zeros, zeros])
#                 cond_mask = torch.zeros(cond_token.shape[:2])
#                 cfg_conditions['description'] = (cond_token, cond_mask)
#             if wav_tokens is not None:
#                 zeros_wav = torch.zeros_like(wav_tokens)
#                 cond_wav_token = torch.cat([wav_tokens, wav_tokens, zeros_wav])
#                 cond_wav_mask = torch.zeros(cond_wav_token.shape[:2])
#                 cfg_conditions['self_wav'] = (cond_wav_token, cond_wav_mask)
...
#         elif (description_tokens is not None) or (wav_tokens is not None):
#             if description_tokens is not None:
#                 if description_tokens_is_cfg:
#                     cond_token = description_tokens
#                 else:
#                     zeros = torch.zeros_like(description_tokens)
#                     cond_token = torch.cat([description_tokens, zeros])
#                 cond_mask = torch.zeros(cond_token.shape[:2])
#                 cfg_conditions['description'] = (cond_token, cond_mask)
#             if wav_tokens is not None:
#                 zeros_wav = torch.zeros_like(wav_tokens)
#                 cond_wav_token = torch.cat([wav_tokens, zeros_wav])
#                 cond_wav_mask = torch.zeros(cond_wav_token.shape[:2])
#                 cfg_conditions['self_wav'] = (cond_wav_token, cond_wav_mask)
...
#             cfg_conditions = {}
#         if self.print_cfg:
#             print("CFG_CONDS", cfg_conditions)
...
#                     double_cfg=double_cfg, triple_cfg=triple_cfg, cfg_coef=cfg_coef, cfg_coef_2=cfg_coef_2, 
#                     cfg_coef_3=cfg_coef_3)
...
#     def _set_print_cfg(self, print_cfg: bool = False):
