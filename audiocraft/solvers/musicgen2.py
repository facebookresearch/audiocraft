...
class MusicGenSolver(base.StandardSolver):
    ...
    def _compute_cross_entropy(
        ...
        The focal loss is computed per codebook to provide codebook-level focal loss.
        ...
        Returns:
            fl (torch.Tensor): Focal loss averaged over the codebooks
            fl_per_codebook (list of torch.Tensor): Focal loss per codebook (detached).
        """
        alpha = 0.05
        gamma = 1.5
        B, K, T = targets.shape
        ...
        fl = torch.zeros([], device=targets.device)
        fl_per_codebook: tp.List[torch.Tensor] = []
        for k in range(K):
            logits_k = logits[:, k, ...].contiguous().view(-1, logits.size(-1))  # [B x T, card]
            targets_k = targets[:, k, ...].contiguous().view(-1)  # [B x T]
            mask_k = mask[:, k, ...].contiguous().view(-1)  # [B x T]
            fl_targets = targets_k[mask_k]
            fl_logits = logits_k[mask_k]
            ce_loss = F.cross_entropy(fl_logits, fl_targets, reduction='none')
            pt = torch.exp(-ce_loss)
            focal_loss = alpha * (1 - pt) ** gamma * ce_loss
            q_fl = focal_loss.mean()
            fl += q_fl
            fl_per_codebook.append(q_fl.detach())
        # average focal loss across codebooks
        fl = fl / K
        return fl, fl_per_codebook
